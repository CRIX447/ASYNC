...
export class LevelBuilder {
      ...
      for (let c = 0; c < width; c++) {
        ...
        switch (ch) {
          ...
          case 'b':
            ...
            break;
          case 'X': {
            // Work out which way the doorway actually faces. Walls to the
            // north and south mean you travel east-west through it, so the
            // panel has to be thin on X -- not the other way round.
            const solid = (rr, cc) => {
              const ch = grid[rr]?.[cc];
  ...
  _makeSwitch(id, center, result) {
    ...
    return {
      ...
      /** Brief red pulse so a wrong order is visibly your fault, not a bug. */
      flashError() {
  ...
  _makeExit(center, cs, wh, result, facesEastWest) {
    ...
    // facesEastWest: you walk through it along X, so the slab is thin on X.
    ...
    // Sign sits on the face you approach from, on whichever axis that is.
    ...
    // A matching sign on the far side, so it reads as a door from both ways.
    ...
    // Interaction target so looking at it tells you why you can't pass.
    const hit = new THREE.Mesh(
    ...
    return {
      ...
      open() {
        ...
        // Green once it's live, and stop advertising itself as sealed.
      ...
      update(dt) {
        ...
        // Sinks into the floor. Slow enough to feel heavy.
