...
export class HUD {
  ...
  setNetState({ online, players, room }) {
    this.el.netstate.innerHTML = online
      ? `<div>${room ? 'ROOM ' + room : 'ONLINE'}</div><div>${players} IN SESSION</div>`
   ...
   */
  showStart({ missingAssets = 0, defaultRoom = 'ASYNC', onStart }) {
    ...
    o.innerHTML = `
      <div class="menu-inner">
        ...
        <div class="panel" data-panel="online">
          <p class="brief">
            Hosted on Photon Cloud — nothing for you to run.<br>
            Share the room code and you're in the same session.<br>
            Voice is push-to-talk on <b class="key">V</b> and fades with distance.
          </p>

          <div class="field">
            <label>ROOM CODE</label>
            <input id="room-code" type="text" value="${defaultRoom}" maxlength="12" spellcheck="false" />
          </div>

          <button class="btn" id="start-online"><span>JOIN ROOM</span></button>
          <div class="status" id="net-status">FREE PHOTON ACCOUNT REQUIRED</div>
        </div>
    ...
    online.addEventListener('click', async () => {
      ...
      status.textContent = 'CONNECTING TO PHOTON…';
      online.disabled = true;

      const res = await onStart('photon', input.value.trim());

      if (res.ok) {
        o.classList.add('hidden');
      } else {
        // Stay on the menu and say exactly why, rather than dumping them into
        // a silently-offline game to wonder where everyone is.
