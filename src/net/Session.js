...
import { PhotonSession } from './PhotonSession.js';
 ...
 */
export async function createSession(sequence, mode = 'solo', arg = null) {
  // Photon Cloud: no server of your own, anywhere.
  if (mode === 'photon') {
    const ph = new PhotonSession(sequence, arg);
    await ph.connect();
    return ph.online ? ph : { failed: true, error: ph.lastError };
  }

  // Your own Node server (npm run server). Kept for local testing.
  if (mode === 'online') {
    const net = new NetworkSession(sequence.length, arg);
    await net.connect();
    return net.online ? net : { failed: true, error: 'COULD NOT REACH SERVER' };
  }
...
export function setSessionSequence(session, sequence) {
  if (typeof session.setSequence === 'function') {
