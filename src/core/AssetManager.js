...
import { TEXTURES, MODELS, PLAYER, MONSTER } from '../config/manifest.js';
...
export class AssetManager {
  constructor() {
    ...
    this.monsterModel = null; // { scene, animations } or null
    this.missing = [];
  ...
  async loadAll(onProgress = () => {}) {
    ...
    const total = texKeys.length + modelKeys.length + 2;
    let done = 0;
    ...
    await Promise.all([
      ...
      this._loadRig('playerModel', PLAYER).then(tick),
      this._loadRig('monsterModel', MONSTER).then(tick)
    ]);
  ...
  /** Shared loader for the rigged player and monster models. */
  async _loadRig(slot, def) {
    try {
      const gltf = await this._gltfLoader.loadAsync(def.url);
      ...
      gltf.scene.traverse((o) => {
        if (o.isMesh || o.isSkinnedMesh) {
          o.castShadow = true;
          o.receiveShadow = true;
          // Skinned meshes get culled wrongly from their bind-pose bounds.
          o.frustumCulled = false;
        }
      });

      this[slot] = { scene: gltf.scene, animations: gltf.animations || [] };
      console.info(
        `${slot} loaded. Animation clips:`,
